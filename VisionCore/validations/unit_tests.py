...
class TestYoloWrapperUltralytics(unittest.TestCase):
    ...
    def test_convert_ultralytics_result_multiple_boxes(self):
        ...
        r = w._convert_ultralytics_to_results(fake_result)
...
class TestConvertModel(unittest.TestCase):
    ...
    def test_skips_conversion_if_not_pt(self):
        from VisionCore.vision.genericYolo import _convert_model
        # Non-.pt files should pass through unchanged regardless of target
        result = _convert_model("model.onnx", "rknn", (320, 320))
    ...
    def test_returns_existing_file_without_calling_export(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch("VisionCore.vision.genericYolo.YOLO") as mock_yolo:
                result = _convert_model(pt, "rknn", (320, 320))
    ...
    def test_convert_model_exports_pt_when_target_file_missing(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            pt = os.path.join(d, "model.pt")
            target = os.path.join(d, "model.rknn")
            open(pt, "w").close()

            with patch.object(sys.modules["ultralytics"], "YOLO") as mock_yolo:
                def export_side_effect(**kwargs):
                    open(target, "w").close()
                    return target

                mock_yolo.return_value.export.side_effect = export_side_effect
                result = _convert_model(pt, "rknn", (320, 320))

            mock_yolo.assert_called_once_with(pt)
